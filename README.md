...
- `assets/paper/mint-paper.pdf` — current manuscript PDF (arXiv pending)
- `ASSET_SOURCES.md` — provenance of every displayed image and video

The film prefers the smaller 720p WebM encode in browsers that support it and
keeps a 720p H.264 MP4 as a compatibility fallback. The six challenging
examples are loaded and played only when they are near the viewport, so opening
the page does not download the entire case grid up front.

All displayed raster imagery comes from the manuscript figures or from
`MINT_v2_revised_music_enhanced_compact.mp4`. Dataset and camera-intrinsic
